package ru.feymer.fmdontinteractionaddon.utils;
...
public class Updater {
    ...
    public Updater(FmDontInteractionAddon plugin) {
    ...
    private boolean checkForUpdates() {
        ...
        try {
            ...
            Bukkit.getConsoleSender().sendMessage(Hex.color("&fПроизошла ошибка при проверке наличия обновлений: &9" + var18.getMessage()));
            return false;
    ...
    public void start() {
        if (this.checkForUpdates()) {
            ...
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9"));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9» &fНайдена новая версия плагина: &9v" + newVersion));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9» &fСкачайте ее по ссылке: &9" + resourceURL));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9"));

        } else {

            Bukkit.getConsoleSender().sendMessage(Hex.color("&9"));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9» &fОбновления не найдены!"));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9» &fВы используете последнюю версию плагина - &9v" + latestVersion));
            Bukkit.getConsoleSender().sendMessage(Hex.color("&9"));
